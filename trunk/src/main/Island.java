...
//					e.printStackTrace();
    ...
    public void removeBlock(Class type)
    {
        for(int i = 0; i < blocks.length; i++)
        {
            for(int j = 0; j < blocks[0].length; j++)
            {
                if(IDManager.getBlockClass(blocks[i][j]) == type)
                {
                    blocks[i][j] = 0;
                }                    
            }
    ...
    public void expand(int x, int y) {
        ...
        blocks = buffer;
        if (left) {
            this.x -= expansionPower * world.BLOCK_SIZE;
        }
        if (up) {
            this.y -= expansionPower * world.BLOCK_SIZE;
        }
    }
